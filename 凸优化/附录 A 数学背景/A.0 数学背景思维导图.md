...
- 向量内积：$\langle x,y\rangle=x^Ty$；欧几里得范数：$\lVert x\rVert_2=(x^Tx)^{1/2}$。[[欧几里得范数]]
...
- 反向构造：$\lVert x\rVert_C=\inf\{t>0\mid x\in tC\}$。[[范数]]
...
- $f:A\to B$只说明输入/输出环境；真实输入满足$\operatorname{dom}f\subseteq A$。[[记号]]
